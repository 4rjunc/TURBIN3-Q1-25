mod programs;
use crate::programs::Turbin_prereq::{ CompleteArgs, Turbin3PrereqProgram, UpdateArgs };
...
mod tests {
    ...
    use crate::programs::Turbin_prereq::{ CompleteArgs, Turbin3PrereqProgram };
    ...
    #[test]
    fn base58_to_wallet() {
        println!("Input your private key in base58");
        let stdin = io::stdin();
    ...
    fn transfer_sol() {
        ...
        let keypair = read_keypair_file("dev-wallet.json").expect("Couldn't find wallet file");
        let client = RpcClient::new(RPC_URL);
        let to_pubkey = Pubkey::from_str("6X4G9p5kiE6tDXkBHfpqispJ2B6YfAA3tBGcKvaXaht2").unwrap();
